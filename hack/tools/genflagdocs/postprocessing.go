...
const genFlagDocsRepoPath = "https://github.com/karmada-io/karmada/tree/master/hack/tools/genflagdocs"
...
// cleanupForIncludeBody returns markdown prepared for website include up to (but not including) the auto-generated footer.
func cleanupForIncludeBody(md string) string {
...
func cleanupForInclude(md string) string {
	var cleanMd strings.Builder
	cleanMd.WriteString(cleanupForIncludeBody(md))
	cleanMd.WriteString("###### Auto generated by [spf13/cobra script in Karmada](" + genFlagDocsRepoPath + ")")
	return cleanMd.String()
}

// cleanupForIncludeWithIndex is like cleanupForInclude but inserts a link to the grouped command index before the footer.
func cleanupForIncludeWithIndex(md string, indexBasename, indexLinkText string) string {
	var cleanMd strings.Builder
	cleanMd.WriteString(cleanupForIncludeBody(md))
	cleanMd.WriteString("\n\n#### Go Back to [" + indexLinkText + "](" + indexBasename + ") Homepage.\n")
	cleanMd.WriteString("\n\n###### Auto generated by [spf13/cobra script in Karmada](" + genFlagDocsRepoPath + ")")
	return cleanMd.String()
}
